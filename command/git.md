...
**git** 是一个**分布式版本控制系统**，用于追踪文件变化、协调多人协作开发。它广泛应用于软件开发、系统运维、文档管理等领域，是目前最流行的版本控制工具。

> 注：2020 年 10 月 GitHub 已将默认分支 `master` 更名为 `main` 分支。

## 补充说明

**git 命令** 不仅仅是一个版本控制工具，它是一个完整的分布式版本控制系统。通过 git，开发者可以记录项目历史、管理分支、协作开发、回滚错误，是现代软件开发不可或缺的基础设施。

git 的工作流程基于以下核心概念：

- **工作区（Working Directory）**：你在电脑里能看到的目录。
- **暂存区（Staging Area）**：介于工作区和版本库之间，用于暂存修改。
- **版本库（Repository）**：git 的核心，存储所有提交记录和分支信息。
...
### 选项
    ...
    --version               显示 git 版本信息
    --help                  显示帮助信息
    -C <path>               在执行命令前切换到指定目录
    -c <name>=<value>       传递配置参数
    --exec-path[=<path>]    获取或设置 git 核心程序路径
    --html-path             显示 git HTML 帮助文档路径
    --man-path              显示 man 帮助文档路径
    --info-path             显示 info 帮助文档路径
    -p / --paginate         通过分页器输出
    -P / --no-pager         不通过分页器输出
    --git-dir=<path>        指定 .git 目录路径
    --work-tree=<path>      指定工作目录路径
    --bare                  将 git 视为裸仓库
...
| 参数 | 说明 |
| --- | --- |
| `command` | 要执行的 git 子命令（如 `init`、`clone`、`add`、`commit` 等） |
| `args` | 子命令所需的参数 |
| `<path>` | 指定的文件或目录路径 |
| `<name>=<value>` | 配置参数的键值对 |
...
- [git mv 移动/重命名文件](#git-mv-移动重命名文件)
...
git 可以使用别名来简化一些复杂命令，类似 [alias](https://github.com/liuzi6612/linux-manual#alias) 命令。
...
```bash
# https 协议克隆
git clone https://github.com/liuzi6612/git-manual.git

# SSH 协议克隆
git clone git@github.com:liuzi6612/git-manual.git

# 克隆指定分支， -b 指定分支名字，实际上是克隆所有分支并切换到 develop 分支上
git clone -b develop https://github.com/liuzi6612/git-manual.git

# --single-branch 完全只克隆指定分支
git clone -b develop --single-branch https://github.com/liuzi6612/git-manual.git

# 指定克隆后的文件夹名称
git clone https://github.com/liuzi6612/git-manual.git git-study # 如果后面是 . 在当前目录创建

# 递归克隆，如果项目包含子模块就非常有用
git clone --recursive https://github.com/liuzi6612/git-manual.git

# 浅克隆, 克隆深度为1, 只克隆指定分支且历史记录只保留最后一条, 通常用于减少克隆时间和项目大小
git clone --depth=1 https://github.com/liuzi6612/git-manual.git
git clone --depth=1 --no-single-branch https://github.com/liuzi6612/git-manual.git # --no-single-branch 同时克隆其他所有分支

# 裸克隆, 没有工作区内容，不能进行提交修改，一般用于复制仓库
git clone --bare https://github.com/liuzi6612/git-manual.git

# 镜像克隆, 也是裸克隆, 区别于包含上游版本库注册
git clone --mirror https://github.com/liuzi6612/git-manual.git
```

### 克隆指定文件夹

有些仓库会包含客户端、服务端等多个端的代码，但又不想完整克隆整个项目，只想克隆某个文件夹，这个时候就需要用到 `稀疏检出`。
...
# 3、设置仓库地址
git remote add origin https://github.com/liuzi6612/git-manual.git
...
git remote add example https://github.com/liuzi6612/git-manual.git
...
git remote set-url origin git@github.com:liuzi6612/git-manual.git
...
```bash
git log -n 1 --pretty=format:"%an" # liuzi6612

git log -n 1 --pretty=format:"%ae" # liuzi6612@gmail.com
...
git remote add upstream https://github.com/liuzi6612/git-manual.git
...
# 比较2个分支指定文件内容差异, develop 和 main README.md 文件差异
...
```bash
git submodule add https://github.com/liuzi6612/git-manual.git # 默认添加到当前目录下
git submodule add https://github.com/liuzi6612/git-manual.git submodules/git-manual  # 添加到指定目录

# -b 指定需要添加仓库的某个分支
git submodule add -b develop https://github.com/liuzi6612/git-manual.git
```
...
如果嫌麻烦每次 git pull 都需要手动添加 `--recurse-submodules`，可以配置 git pull 的默认行为，如何配置请参考 [配置](#git-config-配置)
...
```bash
git subtree add --prefix=sub/common https://github.com/liuzi6612/git-manual.git main --squash
```
...
```bash
git subtree pull --prefix=sub/common https://github.com/liuzi6612/git-manual.git main --squash
```
...
git commit -m "子仓库修改"
# 然后推送
git subtree push --prefix=sub/common https://github.com/liuzi6612/git-manual.git main --squash
```
...
```bash
# common 是仓库名字，可以随意定义
git remote add -f common https://github.com/liuzi6612/git-manual.git
```
...
还有另一种解决方案，就是使用别名，例如在 `mac` 或 `linux` 中使用 [`alias`](https://github.com/liuzi6612/linux-manual#alias) 命令：

```bash
alias push="git subtree push --prefix=sub/common https://github.com/liuzi6612/git-manual.git main --squash"
```
...
{
  "scripts": {
    "push": "git subtree push --prefix=sub/common https://github.com/liuzi6612/git-manual.git main --squash"
  }
}
```
...
旧仓库 A: https://github.com/liuzi6612/A.git
新仓库 B: https://github.com/liuzi6612/B.git

1、克隆旧裸仓库

```bash
# 克隆裸仓库，里面没有工作区内容
git clone --bare https://github.com/liuzi6612/A.git
```
...
```bash
cd A
git push --mirror https://github.com/liuzi6612/B.git
```
...
```bash
git clone https://github.com/liuzi6612/B.git
```
...
```bash
# 公有仓库
git clone https://ghproxy.com/https://github.com/liuzi6612/git-manual.git
...
```bash
https://raw.githubusercontent.com/liuzi6612/git-manual/main/media/poster.png
# 替换为
https://cdn.jsdelivr.net/gh/liuzi6612/git-manual@main/media/poster.png
